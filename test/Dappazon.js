...
describe("Dappazon", () => {
  ...
  describe("Deployment",()=>{
    ...
    expect(name).to.equal("CryptoCart");
  })
  })
  describe("Listing",()=>{
    let trasnaction ; 
    beforeEach(async() =>{
      trasnaction =await dappazon.connect(deployer).list(
        ID,
        NAME,
        CATEGORY,
        IMAGE,
    ...
    it("Emits list event",()=>{
      expect(trasnaction).to.emit(dappazon,"List")
    })
  })
  describe("Buying", () => {
    let transaction
    ...
    it("Adds the order", async () => {
      ...
      expect(order.id).to.be.greaterThan(0)
